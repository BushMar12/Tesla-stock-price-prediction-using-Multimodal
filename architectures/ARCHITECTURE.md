...
This project predicts Tesla next-day stock return using a multimodal sequence-learning pipeline. The main idea is to represent the market as two aligned temporal streams: a price/technical stream and a sentiment stream. Each stream is encoded separately, fused, and then used to predict the next-day return, which is reconstructed into a next-day price.
...
flowchart TD
    ...
    R --> S["Next-Day Return Regression Head"]
    S --> V["Next-Day Price Reconstruction"]
```
...
The training split is chronological rather than random, which is appropriate for time-series forecasting. The target is the next-day return rather than raw price.

## Model Layer

The main neural model is `MultimodalFusionModel`.

The price branch uses a Transformer-based time-series encoder with sinusoidal positional encoding and attention pooling. This branch learns temporal patterns from OHLCV, technical indicators, market context, and calendar features across the 60-day window.
...
The fused representation is passed to one output head:

- a next-day return regression head

## Training Layer

The training loop uses AdamW, ReduceLROnPlateau scheduling on validation MAE, gradient clipping, and early stopping. The active loss is:

- Smooth L1 loss for next-day return regression

During evaluation, predicted scaled returns are inverse-transformed and converted into reconstructed next-day prices. RMSE, MAE, and MAPE are calculated on dollar-price predictions.
...
The Streamlit app follows the same core inference pattern, then presents charts, next-day prediction cards, model comparison, and SHAP explainability.
